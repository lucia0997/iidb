import { AxiosHttpClient } from '@df/utils';
import { AttachErrorConfig } from './types';
import { AxiosError, CanceledError } from 'axios';

export function attachErrorInterceptors(client: AxiosHttpClient, config: AttachErrorConfig) {
  ...
  const responseId = client.addResponseInterceptor(
    async (res) => res,
    async (error: AxiosError | CanceledError | any) => {
