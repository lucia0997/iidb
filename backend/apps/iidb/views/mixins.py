...
def parse_and_validate_columns(request, available_columns: set[str]) -> list[str] | None:
    """
    Read ?columns=a,b,c. Validate against available_columns.
    Return a list (deduplicated, maintaining order) or None if the parameter was not passed.
    Throw ValidationError if there are invalid columns.
    """
    ...
    # Divide, clean and deduplicate while maintaining order
    seen = set()
    cols = []
    for c in (x.strip() for x in raw.split(",") if x.strip()):
        if c not in available_columns:
            raise ValidationError({
                "columns": f"'{c}' is not a valid column.",
